...
- **Хранение данных**: localStorage (обёртка-репозиторий c JSON-сериализацией) или indexDB.
...
1. **Создание списка**:
   - Страница /import: форма + drag&drop CSV (поля: name, description?, image? URL).
   - Ручное добавление: форма добавления элемента (название обязательно).
   - Предпросмотр списка (таблица, счётчик).
   - Валидация (минимум 2 элемента, уникальные id). ID можно генерировать автоматически
2. **Настройка турнира**:
